...
#remove NAs
dat <- dat[!is.na(dat[,64]),]

treatment <- dat$treament
orderModeled <- dat$plant
dat <- dat[,grep("^Otu\\d+$", names(dat))]
dim(dat)
#21 taxa, as expected, however some of these have zero counts in the data. 
...
results_with_Afulva <- modelRun(dat, treatment)
save.image("./withAfulva_BayesianRelativeAbundanceModelingFUNGInoplant_with_p_theta.RData")

# Model again without A. fulva
afulva <- which(names(dat) %in% c("Otu9", "Otu6"))
#Doublechecked that these Otus corresponded with A. fulva Dec 13, 2018

dat <- dat[,-afulva]
#Sanity check
names(dat)

#Also remove the odd sample that had no reads, a control
rmv <- which(rowSums(dat) == 0)
if(length(rmv) > 0){
  dat <- dat[-rmv,]
  treatment <- treatment[-rmv]
  dat <- dat[-rmv]
}
dim(dat)
orderModeled_less <- orderModeled[-rmv]
results_withOut_Afulva <- modelRun(dat, treatment)

save.image("./withOUTAfulva_BayesianRelativeAbundanceModelingFUNGInoplant_with_p_theta.RData")

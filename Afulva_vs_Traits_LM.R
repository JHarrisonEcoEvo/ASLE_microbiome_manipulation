...
dat2 <- read.csv("./data/ML_fungi_ALFU_withShannonSimpson.csv", stringsAsFactors = F)

#Bring in trait data
dat5 <- read.csv("./data/alldata2.csv", stringsAsFactors = F)
dat5 <- dat5[order(dat5$treament),]
dat5 <- dat5[which(dat5$sequenced == "yes"),]
dim(dat5)
dim(dat2)
dat <- merge(dat2, 
      dat5,
...
dat5 <- dat4
test <- rfImpute(x = dat5[,5:11], y = dat5$Afulva1)
dat5[,5:11] <- test[,2:8]

starts <-  bounder(dat5$treatment)[1]
ends <- bounder(dat5$treatment)[2]

modelRun(x = dat5$Afulva1,
         datz = dat5,
         starts = unlist(starts), 
         ends = unlist(ends))

dat5 <- dat4
test <- rfImpute(x = dat5[,5:11], y = dat5$Afulva2)
dat5[,5:11] <- test[,2:8]

starts <-  bounder(dat5$treatment)[1]
ends <- bounder(dat5$treatment)[2]

modelRun(x = dat5$Afulva2,
         datz = dat5,
         starts = unlist(starts), 
         ends = unlist(ends))

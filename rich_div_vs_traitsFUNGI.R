...
return(list( intercept = results(sim.mod.sam$mu,8),
       vol = results(sim.mod.sam$beta1,8),
       sla = results(sim.mod.sam$beta2,8),
       N = results(sim.mod.sam$beta3,8),
       C = results(sim.mod.sam$beta4,8),
       ndfa = results(sim.mod.sam$beta5,8))
      )
}
...
out <- modelRun(x = dat5$shannon,
         datz = dat5,
         starts = unlist(starts), 
         ends = unlist(ends))

resultMat <- matrix(nrow=9,ncol=6)
colnames(resultMat) <- names(out)
for(i in 1:6){
  resultMat[,i] <- out[[i]][,3]
}
...
return(list( intercept = results(sim.mod.sam$mu,4),
             vol = results(sim.mod.sam$beta1,4),
             sla = results(sim.mod.sam$beta2,4),
             N = results(sim.mod.sam$beta3,4),
             C = results(sim.mod.sam$beta4,4),
             ndfa = results(sim.mod.sam$beta5,4),
             swain = results(sim.mod.sam$beta6,4))
)
}

#wrangle, impute, etc.

dat4$swain <- as.numeric(dat4$swain)
...
#omit those treatment groups that are asle negative. 
#This is so all those plants without swainsonine, dont bias the 
#estimate of the effect of swainsonine. 
dat5 <- dat5[grep("plus",dat5$treatment),]

starts <-  bounder(dat5$treatment)[1]
ends <- bounder(dat5$treatment)[2]

#Note treatment specification in the output is incorrect because it assumes 8 treatments not 4. Ignore.

out <- modelRun_swain(x = dat5$shannon,
         datz = dat5,
         starts = unlist(starts), 
         ends = unlist(ends))
